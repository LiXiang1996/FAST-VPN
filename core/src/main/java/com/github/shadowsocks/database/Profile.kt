            ...
            pattern.findAll(data ?: "").map {
                ...
                try {
                    if (uri.userInfo == null) {
                        val match = legacyPattern.matchEntire(
                            String(
                                Base64.decode(
                                    uri.host,
                                    Base64.NO_PADDING
                        ...
                        if (match != null) {
                            ...
                            try {
                                ...
                                if (profile.host.firstOrNull() == '[' && profile.host.lastOrNull() == ']') {
                                    profile.host =
                                        profile.host.substring(1, profile.host.length - 1)
                                }
            ...
            private fun tryParse(json: JsonObject, fallback: Boolean = false): Profile? {
                ...
                return Profile().also {
                    ...
                    (json["proxy_apps"] as? JsonObject)?.also {
                        ...
                        individual = (it["android_list"] as? JsonArray)?.asIterable()
                            ?.mapNotNull { it.optString }
                            ?.joinToString("\n") ?: individual
                    }
                    udpdns = json["udpdns"].optBoolean ?: udpdns
                    (json["udp_fallback"] as? JsonObject)?.let { tryParse(it, true) }
                        ?.also { fallbackMap[this] = it }
                }
            }
            ...
            fun process(json: JsonElement?) {
                when (json) {
                    is JsonObject -> {
                        ...
                        if (profile != null) add(profile) else for ((_, value) in json.entrySet()) process(
                            value
                        )
                    }
