...
class RemoteTestActivity : BaseActivity() {
    ...
    override fun initView() {
        ...
        text1?.text = "广告是否为空${App.remoteADListData == null} isUseLocale${TestVa.isUseLocaleData}"
        ...
        text2?.text = "cache广告 open id $a   chapingid $ab "
        text3?.text = "severliset${TestVa.serverList}"
        text4?.text = "seversmartliset${TestVa.serversmartList}"
        text6?.text = "adlist${TestVa.adlist}"
        text5?.text = "remote  ${TestVa.remote}\n robvn_sm${TestVa.remote?.robvn_sm} \n   robvn_i_2R size${TestVa.remote?.robvn_i_2R?.size} GetJsonData show${GetJsonData.getData(this)?.robvn_sm}"
...
object TestVa{
    ...
    var isUseLocaleData:Boolean = false
}